...
class TeleVuer:
    def __init__(self, use_hand_tracking: bool, binocular: bool=True, img_shape: tuple=None, display_fps: float=30.0,
        ...
        self.camera_event_count_shared = Value('i', 0, lock=True)
        self.controller_event_count_shared = Value('i', 0, lock=True)
        self.hand_event_count_shared = Value('i', 0, lock=True)
        self.last_event_error_shared = Array(ctypes.c_char, 256, lock=True)
        if self.use_hand_tracking:
    ...
    def close(self):
        ...
        if getattr(self, "_atexit_close_registered", False):
            try:
        ...
        if process is not None:
            try:
                ...
                print(f"[TeleVuer] Failed to stop Vuer process cleanly: {exc}", flush=True)
            finally:
                try:
                    process.close()
                except Exception:
    ...
    async def on_cam_move(self, event, session, fps=60):
        try:
            ...
            with self.camera_event_count_shared.get_lock():
                self.camera_event_count_shared.value += 1
            self._clear_event_error()
        except Exception as exc:
            self._record_event_error("CAMERA_MOVE", exc)
    ...
    async def on_controller_move(self, event, session, fps=60):
        ...
        try:
            ...
            def extract_controllers(controllerState, prefix):
                controllerState = controllerState or {}
                # trigger
                ...
                with getattr(self, f"{prefix}_ctrl_thumbstickValue_shared").get_lock():
                    getattr(self, f"{prefix}_ctrl_thumbstickValue_shared")[:] = self._as_vec2(
            ...
            with self.controller_event_count_shared.get_lock():
                self.controller_event_count_shared.value += 1
            self._clear_event_error()
        except Exception as exc:
            self._record_event_error("CONTROLLER_MOVE", exc)
    ...
    async def on_hand_move(self, event, session, fps=60):
        ...
        try:
            ...
            with self.motion_data_ready_shared.get_lock():
                self.motion_data_ready_shared.value = True
            with self.hand_event_count_shared.get_lock():
                self.hand_event_count_shared.value += 1
            self._clear_event_error()

        except Exception as exc:
            self._record_event_error("HAND_MOVE", exc)
    ...
    def _clear_event_error(self):
        with self.last_event_error_shared.get_lock():
            self.last_event_error_shared[0] = b"\0"

    def _record_event_error(self, event_name, exc):
        message = f"{event_name}: {type(exc).__name__}: {exc}"
        encoded = message.encode("utf-8", errors="replace")[:255]
        with self.last_event_error_shared.get_lock():
            self.last_event_error_shared[: len(encoded)] = encoded
            if len(encoded) < len(self.last_event_error_shared):
                self.last_event_error_shared[len(encoded)] = b"\0"
        print(f"[TeleVuer] {message}", flush=True)
    ...
    @property
    def camera_event_count(self):
        with self.camera_event_count_shared.get_lock():
            return self.camera_event_count_shared.value

    @property
    def controller_event_count(self):
        with self.controller_event_count_shared.get_lock():
            return self.controller_event_count_shared.value

    @property
    def hand_event_count(self):
        with self.hand_event_count_shared.get_lock():
            return self.hand_event_count_shared.value

    @property
    def last_event_error(self):
        with self.last_event_error_shared.get_lock():
            raw = bytes(self.last_event_error_shared[:])
